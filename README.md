...
Подробная инструкция по запуску и примеры команд: [APP-USAGE.md](./APP-USAGE.md)
...
* Добавлено поле `log_gamma_correction` в JSON-конфиг, добавлен параметр `-lgc`/`--log-gamma-correction` для CLI:
* Булевое значение, которое включает использование логарифмической гамма-коррекции плотностей (по умолчанию включено, соответствует реализации из статьи);
